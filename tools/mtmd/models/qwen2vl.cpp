...
ggml_cgraph * clip_graph_hcx_qwen2vl::build() {
    GGML_ASSERT(model.patch_bias == nullptr);
    GGML_ASSERT(model.class_embedding == nullptr);
    ...
    norm_type norm_t = NORM_TYPE_RMS; // qwen 2.5 vl
    ...
    // Omni extra projector: mm_projector
    embeddings = ggml_mul_mat(ctx0, model.mm_fc_w, embeddings);
    embeddings = ggml_add(ctx0, embeddings, model.mm_fc_b);
